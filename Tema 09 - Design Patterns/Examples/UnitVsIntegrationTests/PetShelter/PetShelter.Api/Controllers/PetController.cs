...
public class PetController : ControllerBase
{
    ...
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<ActionResult<Pet>> Get(int petId)
    {
        var foundPet = await _petService.GetPet(petId);
        if (foundPet is null)
        {
            return NotFound();
        }
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<ActionResult<Pet>> GetPetByName([FromQuery] string petName)
    {
        var foundPet = await _petService.GetPet(petName);
        if (foundPet is null)
        {
            return NotFound();
        }
